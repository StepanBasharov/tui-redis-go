...
Interactive Redis CLI viewer. Go 1.25, module `github.com/StepanBasharov/tui-redis-go`.
...
- `internal/processor/` -- parses user input into `RedisCmdData` and calls the adapter. One file per command: `setCmd.go`, `getCmd.go`, `delCmd.go`, `existsCmd.go`, etc. Tests follow the same split: `setCmd_test.go`, `getCmd_test.go`, `delCmd_test.go`.
...
- Multi-key commands (DEL, EXISTS) store keys in `RedisCmdData.Keys`; single-key commands (GET, SET) use `Key`
- Sentinel errors per package (e.g. `ErrNotEnoughArgs`, `ErrInvalidSetOptions` in processor; `ErrExists`, `ErrDelValue` in adapter)
- Constructor `NewRedisAdapter` takes `context.Context` as first param
- EXPIRE processor handler is a stub (no-op); adapter method has commented-out implementation
